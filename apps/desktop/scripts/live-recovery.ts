...
const settleMs = Number(process.env.QST_LIVE_SETTLE_MS ?? 60000)
...
let ocrIndex = 0
const parseText = TesseractOCRProvider.prototype.parseText
TesseractOCRProvider.prototype.parseText = async function(image) {
  if (image.purpose === 'ASSET') {
    const bitmap = Buffer.from(Uint8Array.from({ length: image.width * image.height * 4 }, (_, i) => i % 4 === 3 ? 255 : image.grayscale[Math.floor(i / 4)]!))
    writeFileSync(join(out, `asset-variant-${++ocrIndex}.png`), nativeImage.createFromBitmap(bitmap, { width: image.width, height: image.height }).toPNG())
  }
  return parseText.call(this, image)
}
...
void app.whenReady().then(async () => {
  ...
  const prepare = async (platform: Platform): Promise<void> => {
    const before = await requestConfiguration(connection, { platform, operation: 'get' })
    const profile = before.calibrations.find(p => p.id === before.activeCalibrationId)
    // This explicit recovery run recalibrates the visible grid when a saved manual rectangle is obsolete.
    if (profile?.geometrySource === 'MANUAL') {
      const resolved = await browsers.resolveGrid(platform)
      if (!resolved.grid) throw new Error('No verified grid')
      const result = await requestConfiguration(connection, { platform, operation: 'saveCalibration', name: 'Live capture recovery',
        geometrySource: 'AUTO', slots: resolved.grid.slots.map(s => ({ id: s.slotId, bounds: s.chartBounds })),
  ...
  try {
    ...
    log('Waiting 30 seconds for the operator to close the portfolio panels')
    await wait(30000)
    for (const platform of platforms) {
      ...
      writeFileSync(join(out, `${platform}-surface.png`), (await browsers.captureSurface(platform)).image.toPNG())
      let state = await assetSync.command({ platform, operation: 'sync' })
      for (let attempt = 1; state.error && attempt < 3; attempt++) {
        log(`${platform}: sync retry after ${state.error}`)
        await wait(1500)
        state = await assetSync.command({ platform, operation: 'sync' })
      }
      platformReport.syncError = state.error
      platformReport.sync = structuredClone(state)
      ...
      platformReport.probe = structuredClone(await market.probe(platform))
      log(`${platform}: probe ${JSON.stringify(platformReport.probe)}`)
    ...
    for (const platform of observing) {
      const snapshot = market.command({ platform, operation: 'state' })
      report.operational = market.operationalState()
