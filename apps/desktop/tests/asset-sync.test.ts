...
it('reports uncertain geometry and never saves a shifted partial mapping', async () => {
  ...
  const state = await manager!.command({ platform: 'capitalbear', operation: 'sync' })
  expect(state.error).toContain('TAB_GEOMETRY_UNCERTAIN')
  expect(state.syncFresh).toBe(false)
  expect(state.syncStatus).toBe('FAILED')
  expect(state.detection).toBeNull()
  expect(save).not.toHaveBeenCalled()
})
it('marks syncFresh true and syncStatus APPLIED on successful sync', async () => {
  setup(async () => result(9))
  const state = await manager!.command({ platform: 'capitalbear', operation: 'sync' })
  expect(state.syncFresh).toBe(true)
  expect(state.syncStatus).toBe('APPLIED')
