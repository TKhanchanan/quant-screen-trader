...
for (const platform of ['capitalbear', 'iqoption'] as const) describe(`${platform} asset detection`, () => {
  ...
  it('uses matching complete tooltips and rejects truncated or conflicting identity', () => {
    ...
    expect(mapChartLabels(platform, [chart(1, 'Injective (OT...')], defaultCalibration()).slots[0]?.assetName).toBeNull()
    expect(mapChartLabels(platform, [chart(1, 'EUR/USD'), chart(1, 'GBP/USD')], defaultCalibration()).slots[0]?.state).toBe('UNCERTAIN')
    expect(normalizeAsset('EUR/USD (OT...')).toBeNull()
    expect(normalizeAsset('Sui (O…')).toBeNull()
    expect(normalizeAsset('AUS 200 (OT... Digital')).toBeNull()
    expect(normalizeAsset('Plati...')).toBeNull()
...
it('rejects OCR garbage and validates plausible asset names', () => {
  ...
  expect(normalizeAsset('EUR/JPY (OTC)')).toBe('EUR/JPY OTC')
  expect(normalizeAsset('nzd / usd (otc)')).toBe('NZD/USD OTC')
